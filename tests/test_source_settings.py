...
def test_ds10_list_never_echoes_raw_credentials_or_query_values(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("BINGGO_HOME", str(tmp_path))
    raw = "https://api-user:super-secret@example.com/lottery.json?token=token-value&uid=123456"
    ...
    assert "super-secret" not in entry["display"]
    assert "token-value" not in entry["display"]
    assert "123456" not in entry["display"]
    assert "***" in entry["display"] or "%2A%2A%2A" in entry["display"]
...
def test_ds10_rejects_unsupported_schemes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("BINGGO_HOME", str(tmp_path))
