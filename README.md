...
- 主页：[space.bilibili.com/492426375/upload/opus](https://space.bilibili.com/492426375/upload/opus)
- 每日 Opus 专栏，自动识别「互动抽奖 / 预约抽奖」分区并写入分类提示
- 已接入一键更新、合并去重、分类、详情拉取、活动列表、参与等全链路
- 新增 `scripts/check_ds6.py` 与 `src/sources/ds6_nuomi.py`

**修复与优化**

- 任务日志脱敏：过滤 Traceback / uvicorn 堆栈，避免成功任务误报「内部错误」
- 一键更新进度条：按数据源数量动态计算（适配 6 个数据源）
- `state.json` 并发写入加锁，避免并行检查数据源时状态丢失
- Windows 下 Dashboard 使用更稳定的事件循环策略
