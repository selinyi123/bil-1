...
<p align="center">
  开源本机 B 站抽奖助手 · 完整 Web 控制台，不是命令行脚本 · Win / macOS<br>
  <em>Local-first open-source Bilibili lottery helper — discover, join, schedule on your machine</em>
</p>
...
抽奖不用自己翻合集、翻动态去找，也不用一条条点参与。Binggo 在浏览器里打开本机控制台：自动发现进行中的活动，一键参与或三连一次点完；需要时挂上定时调度，真正省心。

| | |
|---|---|
| **自动找活动** · Auto-discover | 合集增量同步 + 监控名单补转发漏网 |
| **一键 / 三连** · Join in bulk | 互动、转发、预约同列表；三连一次带走多条未参加 |
...
| **参与方式** | 单条「参与」或「三连参与」（当前筛选下并行最多 3 条未参加） |
| **发现通道** | 内置多路 UP 合集 + 可配置监控用户 MID 转发动态 |
| **调度** | 定时点击监视器：到点自动点指定按钮；与进行中 Job 撞车则停机 |
...
**一键 / 三连** — 互动、转发、预约都在同一张列表里点完；三连按当前筛选一次参与最多 3 条未参加，把「一条条点」收成「一次点完」。

**定时挂着点** — 可选定时更新与参与；到点自动点控制台按钮。若已有任务在跑则撞车即停，不会像无脑脚本硬抢并发。
...
- **定时点击** — 浮动「定时点击监视器」：到点自动点控制台按钮（如更新、参与）
- **启动 / 停止调度** — 显式控制；**撞车即停**，不会取消正在跑的参与 Job
- **任务日志** — 右下角浮动面板，SSE 实时日志（断线回退轮询）
...
按类型/状态/热度筛选，快捷操作区可直接 **三连参与 (3)**；列表支持大规模分页浏览与单条「参与」。详见 [控制台 · 活动](docs/console.md#活动)。

### 三连进度 · 调度监视器 · 任务日志

<p align="center">
  <img src="assets/screenshots/readme/triple-and-scheduler.png" alt="三连参与与定时调度" width="920">
</p>

三连并行时进度、点赞/关注/收藏/转发/评论步骤同屏；右侧定时点击监视器与任务日志让你知道「跑到哪一步、下次何时点」。详见 [控制台 · 浮动面板](docs/console.md#浮动面板全局)。
...
| **批量参与** | 一条条点 | 视脚本而定 | 三连并行最多 3 条 + 单条参与 |
...
3. 在「活动」筛选未参加 → **参与** 或 **三连参与**。转发抽奖需先在概览配置 LLM 并测试通过。
...
可选扩展：通过 **MCP stdio** 让 Agent（如 Cursor）串行调用与网页相同的按钮能力 — 扫码登录、拉活动列表、`更新此源`、参与 / 三连参与、定时调度等。**不修改**主项目业务代码，仅 HTTP 调用本机控制台。
