...
- 重写 Web 扫码登录流程：分阶段状态（等待扫码 → 已扫码 → 确认中 → 成功/失败），手机确认后界面实时反馈
- 修复「扫了码却毫无反应」：前后端二维码 key 不一致（旧图缓存、静默刷新）导致轮询永远等待
- 首次生成二维码即通知前端换图；禁止浏览器缓存二维码；登录前清除过期图片
- 关闭过期自动刷新，避免扫码后会话被静默替换
- Cookie 原子写入；轮询/生成请求网络失败自动重试；校验 `SESSDATA` 与 `bili_jct` 完整落盘
- 支持取消登录、重复点击恢复弹窗、二维码加载失败自动重试
